...
const REVIEW_TONE: Record<ReviewBundleSummary["state"], BadgeTone> = {
  preparing: "neutral",
  ready: "added",
...
const IMPORT_TONE: Record<ImportSessionState, BadgeTone> = {
  pending: "neutral",
  scanning: "accent",
  fingerprinting: "accent",
  grouping: "accent",
  matching: "accent",
  reviewing: "accent",
  completed: "added",
...
function StatTile({
  ...
  return to ? (
    <Link
      ...
      className="focus-ring flex-1 min-w-[140px] rounded-md border border-border-subtle bg-surface-raised p-4 text-inherit no-underline hover:border-border-strong transition-colors"
    >
      {content}
    </Link>
  ) : (
    <div
      aria-label={`${label}: ${value} (informativo)`}
      className="flex-1 min-w-[140px] p-4 border border-dashed border-border-subtle rounded-md bg-surface opacity-90"
    >
      {content}
    </div>
  );
    ...
    <div className="font-sans text-text-primary bg-canvas min-h-0">
      ...
      <div className="p-5 flex flex-col gap-5">
        {summary.isError ? (
          ...
          <div className="flex gap-3 flex-wrap">
            <StatTile
              label="File nel catalogo"
              value={summary.data.total_tracks}
              to="/catalog"
            />
            ...
            <StatTile
              label="Ungrouped"
              value={summary.data.ungrouped_track_count}
            />
            <StatTile
              label="Problemi da risolvere"
              value={summary.data.tracks_with_errors}
              to="/catalog?status=errored"
            />
            <StatTile
              label="Missing art"
              value={summary.data.tracks_missing_art}
              to="/catalog?flags=missing-art"
            />
            <StatTile
              label="File mancanti"
              value={summary.data.tracks_missing}
              to="/catalog?status=missing"
            />
          </div>
        )}
              ...
              <div className="flex flex-col gap-2">
                {recentReviews.data?.pages[0]?.items.map((review) => (
                  ...
                  >
                    ...
                    <Badge tone={REVIEW_TONE[review.state]}>
                      {review.state}
                    </Badge>
                  </Link>
                ))}
              ...
              <div className="flex flex-col gap-2">
                {recentImports.data.items.map((session) => (
                  <Link
                    ...
                    className="flex items-center justify-between gap-3 text-inherit no-underline"
                  >
                    <span className="text-sm">#{session.id} Import</span>
                    <Badge tone={IMPORT_TONE[session.state]}>
                      {session.state}
                    </Badge>
                  </Link>
                ))}
