...
export function ReviewManualSearch() {
  ...
  const [providerSelectionInitialized, setProviderSelectionInitialized] =
    useState(false);
  const [result, setResult] = useState<ManualCandidateSearchResult | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!providerSelectionInitialized && availableProviders.length > 0) {
      setSelectedProviders(
        availableProviders.map((capability) => capability.provider),
      );
  ...
  const search = useMutation({
    mutationFn: (page: number) =>
      searchManualCandidates(reviewId, {
        ...form,
        providers,
        page,
        page_size: 10,
    ...
    onSuccess: (next, page) => {
      setResult((previous) =>
        page > 0 && previous
          ? {
              ...next,
              candidates: [...previous.candidates, ...next.candidates],
            }
          : next,
      );
    },
  ...
  const selectCandidate = useMutation({
    mutationFn: ({
      ...
      refId: string;
      force?: boolean;
    }) => importManualCandidate(reviewId, source, refId, force ?? false),
    onSuccess: (updated) => {
    ...
    onError: (err: unknown, variables) => {
      if (
        ...
        setPendingConfirm({
          source: (variables as { source: string; refId: string }).source,
          refId: (variables as { source: string; refId: string }).refId,
        });
      }
    },
  });
  ...
  if (!Number.isFinite(reviewId)) {
    return (
      <EmptyState
        ...
        description="A numeric review ID is required."
      />
    );
  }
  if (review.isLoading || capabilities.isLoading) {
    return (
      <EmptyState
        ...
        description="Preparing manual search."
      />
    );
  }
  if (review.isError || capabilities.isError || !review.data) {
    return (
      <EmptyState
        ...
        description="This review cannot be searched right now."
      />
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-5 py-6">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <Link
            ...
            className="focus-ring rounded text-sm text-text-secondary"
          >
            Torna alla review
          </Link>
          ...
          <p className="text-sm text-text-secondary mt-1">
            Searches add a candidate to this review; they never change music
            files.
          </p>
        </div>
        {currentCandidate.candidate_source && (
          <Badge tone="added">
            Selected: {providerLabel(currentCandidate.candidate_source)}
          </Badge>
        )}
      </div>
      ...
      >
        <div className="grid gap-3 sm:grid-cols-3">
          <label className="text-sm text-text-secondary">
            ...
            <input
              ...
              className="mt-1 w-full rounded border border-border-subtle bg-bg-base p-2 text-text-primary"
            />
          </label>
          <label className="text-sm text-text-secondary">
            Artist
            <input
              aria-label="Artist"
              value={form.artist ?? ""}
              onChange={(event) => updateText("artist", event.target.value)}
              className="mt-1 w-full rounded border border-border-subtle bg-bg-base p-2 text-text-primary"
            />
          </label>
          <label className="text-sm text-text-secondary">
          ...
          <label className="text-sm text-text-secondary">
            ...
            <input
              ...
              onChange={(event) =>
                updateNumber("duration_ms", event.target.value)
              }
              className="mt-1 w-full rounded border border-border-subtle bg-bg-base p-2 text-text-primary"
            />
          </label>
          <label className="text-sm text-text-secondary">
            ...
            <input
              aria-label="ISRC"
              value={form.isrc ?? ""}
              onChange={(event) => updateText("isrc", event.target.value)}
              className="mt-1 w-full rounded border border-border-subtle bg-bg-base p-2 text-text-primary"
            />
          </label>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          <Button
            type="submit"
            disabled={search.isPending || providers.length === 0}
          >
            {search.isPending ? "Searching…" : "Search"}
          </Button>
          {search.isError && (
            <span className="text-sm text-diff-removed">
      ...
      {result && (
        <section className="mt-6" aria-live="polite">
          ...
          {result.candidates.length === 0 ? (
            <EmptyState
              title="No candidates"
              description="Try a different query, provider, or a supported URL when that option is available."
            />
          ) : (
            <div className="flex flex-col gap-3">
              {result.candidates.map((candidate) => {
                const selected =
                  candidate.source === currentCandidate.candidate_source &&
                  candidate.ref_id === currentCandidate.candidate_ref;
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-text-secondary">
                          {[
                            candidate.representative_artist ??
                        ...
                        <p className="text-2xs text-text-muted mt-1">
                          ...
                          {candidate.track_count ?? "Unknown"} tracks
                        </p>
                      </div>
                      <Button
                        size="sm"
          ...
          {result.has_more && (
            <div className="mt-4">
              <Button
                ...
                onClick={() => submit(result.query.page + 1)}
              >
                Show more
              </Button>
            </div>
              ...
              onClick={() => {
                if (pendingConfirm)
                  selectCandidate.mutate({
                    ...
                    refId: pendingConfirm.refId,
                    force: true,
                  });
              }}
            >
              Sovrascrivi valori manuali
            </Button>
          </>
        }
      >
        Questo candidato sovrascriverà valori modificati manualmente. Confermi?
      </Modal>
    </div>
  );
}
