...
async def test_url_import_uses_cache_fresh_and_reuses(db_session: Session) -> None:
    ...
    mock_provider.capabilities = {__import__("muzilla.providers.base", fromlist=["Capability"]).Capability.GET_RELEASE}
    ...
    ps = ProviderSet(metadata={"musicbrainz": mock_provider}, art={}, lyrics={}, fingerprint={}, clients=())  # type: ignore[arg-type]  # mock provider lacks full protocol
    recognized = CandidateUrlRef(provider="musicbrainz", candidate_type="release", provider_id="mb-url-1")
    config = Config()
    # First call: network
...
async def test_url_import_offline_returns_only_cached(db_session: Session) -> None:
    ...
    mock_provider.capabilities = frozenset({Capability.GET_RELEASE})
    mock_provider.get_release = AsyncMock(return_value=cand)
    ps = ProviderSet(metadata={"musicbrainz": mock_provider}, art={}, lyrics={}, fingerprint={}, clients=())  # type: ignore[arg-type]  # mock
    recognized = CandidateUrlRef(provider="musicbrainz", candidate_type="release", provider_id="mb-offline")
    ...
    mock_provider.get_release.reset_mock()
    result_offline = await _fetch_url_candidate(ps, recognized, session=db_session, config=config_offline)
...
async def test_url_import_stale_fallback_on_failure(db_session: Session) -> None:
    ...
    ps_ok = ProviderSet(metadata={"musicbrainz": mock_ok}, art={}, lyrics={}, fingerprint={}, clients=())  # type: ignore[arg-type]  # mock
    recognized = CandidateUrlRef(provider="musicbrainz", candidate_type="release", provider_id="mb-stale")
    config = Config()
    ...
    ps_fail = ProviderSet(metadata={"musicbrainz": mock_fail}, art={}, lyrics={}, fingerprint={}, clients=())  # type: ignore[arg-type]  # mock
