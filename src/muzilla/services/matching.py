"""Matching service: the only way api/cli reach `muzilla.matching`,
`muzilla.providers`, or `muzilla.changes.builder` for match proposals.

Owns the seam between the DB (`Track`/`TrackGroup` rows) and the pure,
network-free `matching/engine.py` — converts rows to `TrackMeta`,
fetches candidates via the provider set, scores them, and (on request)
stages a `match_proposal` ChangeSet from one chosen candidate. Matching
itself never touches the DB or the network; this module is where those
two worlds meet.
"""
