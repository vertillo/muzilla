...
clients (docs/PLAN.md §2). The only place `api`/`cli` reach
`muzilla.providers` from — neither may import it directly.
...
def build_provider_set(config: Config) -> ProviderSet:
    ...
    Missing tokens degrade gracefully (docs/PLAN.md §8): a provider
