...
def _print_candidates(
    ...
    typer.echo(
        f"decision: {label} (strong={strong} ambiguous={ambiguous}) distance shown per candidate"
    )
    for i, c in enumerate(rows):
        typer.echo(
            f"  [{i}] {c.source} {c.album!r} ref={c.ref_id} distance={c.adjusted_distance:.3f} rejected={bool(c.rejection_reason)}"
        )
    if band == "strong":
        typer.echo(
            "preselected: top candidate is preselected but requires explicit Apply in web UI"
        )
    elif band == "ambiguous":
        typer.echo(
            "action required: select a candidate or Skip / Leave unchanged; unresolved blocks Apply"
        )
...
def match_group(
    ...
    stage: Annotated[str | None, typer.Option(help="Stage candidate source:ref_id")] = None,
) -> None:
    config = load_config()
